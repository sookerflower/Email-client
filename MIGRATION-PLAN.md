...
- **2026-07-26 — Phase 5.1 complete (server beacons: publishBeacon + Redis pub/sub + SSE with ownership auth).** `lib/beacons.ts`: `publishBeacon(connectionId, msg)` → Upstash-REST `PUBLISH beacon:{connectionId}` — workerd-safe on both runtimes (spike-verified through the local proxy before wiring); `MailEngine.broadcast` is its ONLY wrapper and stopped being a no-op. Subscribe side is Node-only by the same hygiene rule as BullMQ: `src/node/realtime.ts` (ioredis TCP subscriber, one shared connection with per-channel listener sets, subscribe-on-first/unsubscribe-on-last) registered from `entry.ts`; **route is `GET /realtime/:connectionId`, NOT `/api/realtime`** — the /api sub-app's tRPC catch-all middleware would intercept unknown /api/* paths ahead of a late-registered handler. SSE: `event: open` handshake, 25 s `:hb` heartbeats, cleanup on both stream-cancel and request-abort. **Ownership auth in from the first leg:** better-auth session (401 without) + Postgres `connection.userId === session.user.id` (403 otherwise; missing and not-owned return the same 403 so ids don't leak). **Publish-after-commit call-site audit** (the guarantee; the probe is only the backstop) — every `broadcast` call site and the write it follows: (1) `syncThread` mail-engine.ts → Mail_Get after `upsertThread` — **awaited**; (2) `modifyThreadLabelsInDB` → Mail_Get after `modifyThreadLabels` — **awaited**; (3) `reloadFolder` → Mail_List, signal-only, all three callers (syncFolderOnce, syncFolderJob, modifyLabels loop) invoke it after their index writes — **awaited**; (4) `deleteThread` → Mail_List(trash) after `deleteIndexedThread` — **awaited**; (5) `getUserTopics` → User_Topics after the Redis cache `set` — **awaited**; (6) `sendDoState` → Do_State — payload-carrying (data rides in the message; no announced write) — **n/a by design**. `routes/agent/index.ts` `broadcastChatMessage` sites are workerd-only ZeroAgent chat frames, dead on Node, deleted in 5.4 — outside beacon scope. New `scripts/e2e-realtime.mjs` (9 legs): auth (401 / two-user 403 GreenMail-only / foreign-id 403 / 200+open), api-process beacon delivery (markAsRead → Mail_Get for that threadId), **concurrent order probe** (4 overlapping mutate → beacon → immediate-refetch cycles per the approval note — a before-commit publish only loses under write contention), worker-process fan-in (SMTP → IDLE → sync job → beacon relayed cross-process; 4.2 s GreenMail / 127 s real), heartbeat, reconnect. Script-soundness note: `waitFor` matches NEW events only (registered before the triggering mutation) — history-scanning waits could be satisfied by stale look-alike beacons from earlier syncs; `includeHistory` exists solely for the `open` handshake. *Long-standing flake ROOT-CAUSED and fixed:* the intermittent `--real` forceSync "fetch failed" was undici's 300 s `headersTimeout` in the E2E scripts vs. multi-minute non-streaming tRPC responses (this run: 282 s) — scripts now set a 900 s dispatcher (undici added as devDep); a first-guess server-side `requestTimeout` change was considered and reverted (it governs request receipt, not response duration). Verified: tsc server **28** / mail **257**; e2e-mail ALL GREEN GreenMail + m.re.cx; e2e-realtime ALL GREEN GreenMail + m.re.cx; folder-label regression green; wrangler dry-run builds (ioredis confined to the Node layer); Google OAuth URL OK.

- **2026-07-26 — Phase 5.2 complete (client beacon hook + reconnect discipline).** `lib/realtime-beacons.ts` (framework-free core: EventSource lifecycle, every open-after-the-first reports a reconnect, CLOSED sources recreated on 3 s backoff while CONNECTING is left to native retry, per-type beacon routing) + `hooks/use-realtime-beacons.tsx` (React wiring). AISidebar's WS `onMessage` removed; `useAgent` stays as a dead stub solely because `useAgentChat` needs the handle until the 5.4 swap. **Deliberate deviation from "move the body unchanged", found by live browser verification:** the old body invalidated `mail.listThreads` by exact input triple `{folder, labelIds, q}` — that code path never actually ran on Node, and in the browser its beacon invalidations missed the mounted list query (badge updated, list didn't). Invalidation is now PREDICATE-based on the procedure key (`['mail','listThreads']`, folder-scoped via the key's input; reconnect blanket = all listThreads variants + all `mail.get`), which is immune to input-shape drift by construction; the per-type routing itself is unchanged. Unit test `packages/testing/unit/realtime-beacons.spec.ts` (9 tests) locks: per-type routing incl. Do_State → jotai (not an invalidation), malformed-payload swallow, first-open-is-not-a-reconnect, fatal-error → recreate → reopen fires the blanket reconnect, native-retry reopen also fires it, close() stops everything. **Browser pass (Layer C; evidence is DOM-tree + network-log — the pane's screenshot capture times out on this app):** (1) live inbox update — SMTP-delivered message appeared at the top of the list with zero interaction; (2) **reconnect-repaint** — api killed, message delivered while down (worker synced it to Postgres; its beacon had no relay), api restarted → network log shows EventSource `ERR_CONNECTION_REFUSED` retries then 200, and the offline message appeared via the blanket invalidation alone; (3) two tabs — both tabs' lists updated live from one SMTP delivery (per-tab SSE streams, pub/sub fan-out). **Operational finding during `--real` verification: E2E mailbox bloat.** Every real run had been leaving 3–5 messages on m.re.cx forever; the inbox hit 55 threads, full-refetch syncs stretched past the 240 s windows, and both real suites started flaking (idle-push, worker-fanin). Fixed at the root instead of widening windows again: a one-off purge hard-deleted 34 accumulated e2e threads (inbox 55→23; forceSync fell 282 s→72 s, worker-fanin 127 s→8 s), and both scripts now end their `--real` runs with a best-effort `cleanup` leg that hard-deletes the run's own threads via the worker /rpc driver (thread delete sweeps every folder member) — the accumulation is stopped, not just paid down. GreenMail needs none of this (drop-recover wipes it). Verified: tsc server **28** / mail **257** (one interim +1 from a Do_State type mismatch, fixed); unit tests 27/27; e2e-mail + e2e-realtime ALL GREEN on GreenMail and m.re.cx (post-purge, with cleanup legs); wrangler dry-run builds; Google OAuth URL OK.
